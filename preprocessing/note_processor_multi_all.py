...
def process_line(row):
    ...
    # Construct new row
    return row[:2] + [row[-1], keywords, row[-3]]
...
def line_generator3(filename, chunksize=1):
...
def main(note_path, dictionary_path, output_file_path,max_window=5,threshold=0.8):
    ...
    with Pool(processes=NUM_WORKERS, initializer=initialize_nlp, initargs=(args,)) as pool:
        ...
        #results = list(tqdm(pool.imap_unordered(process_line, (row for row in line_generator(note_path) if row[-1].isdigit() and int(row[-1])> 5 )), total=number_lines, desc="Processing lines"))
        ...
        results = list(tqdm(pool.imap_unordered(process_line, (row for row in line_generator3(note_path) if row[-1] != 'nb_words' and int(row[-1]) > 5 )), total=number_lines, desc="Processing lines"))
...
if __name__ =='__main__':
    ...
    for th in thresholds:
        for w in windows:
            ...
            note_name = note_path.split('/')[-1].replace('.csv','')
            output_file = os.path.join(output_path, f'results_{note_name}_{th}_{w}.csv')
