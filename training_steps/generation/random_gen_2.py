...
def quick_random_sample(file_path, sample_size=10000):
    # Get total line count
    with open(file_path, 'r') as f:
        total_lines = sum(1 for line in f)

    # Generate random sample lines (excluding header)
...
def sample_keywords(csv_path: str, num_samples: int = 10, max_codes: int = 1, max_kws: int = 1, seed: int = 42):
    ...
    for i in tqdm(range(num_samples)):
        ...
        code_principal = df_sample.iloc[i]['diagnostic'].strip()
        code_associes = df_sample.iloc[i]['diagnostic_associes'].strip().split()
        n_codes = random.randint(1, min(max_codes,len(code_associes)))

        #df_sample_codes = [df_sample.iloc[i]['column diagnostic'].strip()] + random.sample(df_sample.iloc[i]['column diagnostic_associes'].strip().split(), n_codes)
        sampled_codes = [code_principal] +  random.sample(code_associes, n_codes)
        sampled_codes = [remove_symbols(c) for c in sampled_codes]

        sampled_kws = []
        for c in sampled_codes:
            if not c in code2key:
                continue
